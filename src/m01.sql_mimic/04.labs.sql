...
-- Labs
DROP TABLE IF EXISTS r_policytree_mimic_2023.labs;
CREATE TABLE r_policytree_mimic_2023.labs AS
WITH cbc0 AS (
    ...
    FROM r_policytree_mimic_2023.cohort AS pt
             INNER JOIN `physionet-data.mimiciv_derived.complete_blood_count` AS ce
    ...
    FROM r_policytree_mimic_2023.cohort AS pt
             INNER JOIN aims_mimiciv_derived.chemistry AS ce
                        ON pt.subject_id = ce.subject_id AND
                           pt.hadm_id = ce.hadm_id
    WHERE pt.disch_dt >= ce.charttime
      -- AND ce.charttime >= pt.admit_dt
    ...
    FROM r_policytree_mimic_2023.cohort AS pt
             INNER JOIN `physionet-data.mimiciv_derived.blood_differential` AS ce
                        ON pt.subject_id = ce.subject_id AND
                           pt.hadm_id = ce.hadm_id
    WHERE pt.disch_dt >= ce.charttime
      -- AND ce.charttime >= pt.admit_dt
    ...
    SELECT ce.subject_id,
           ...
           ce.ptt        AS coag_ptt
    FROM r_policytree_mimic_2023.cohort AS pt
             INNER JOIN `physionet-data.mimiciv_derived.coagulation` AS ce
                        ON pt.subject_id = ce.subject_id AND
                           pt.hadm_id = ce.hadm_id
    WHERE pt.disch_dt >= ce.charttime
      -- AND ce.charttime >= pt.admit_dt
    ...
    FROM r_policytree_mimic_2023.cohort AS pt
             INNER JOIN `physionet-data.mimiciv_derived.enzyme` AS ce
                        ON pt.subject_id = ce.subject_id AND
                           pt.hadm_id = ce.hadm_id
    WHERE pt.disch_dt >= ce.charttime
      -- AND ce.charttime >= pt.admit_dt
    ...
    FROM r_policytree_mimic_2023.cohort AS pt
             INNER JOIN `physionet-data.mimiciv_derived.cardiac_marker` AS ce
                        ON pt.subject_id = ce.subject_id AND
                           pt.hadm_id = ce.hadm_id
    WHERE pt.disch_dt >= ce.charttime
      -- AND ce.charttime >= pt.admit_dt
    ...
    FROM r_policytree_mimic_2023.cohort AS pt
             INNER JOIN `physionet-data.mimiciv_derived.bg` AS ce
                        ON pt.subject_id = ce.subject_id AND
                           pt.hadm_id = ce.hadm_id
    WHERE pt.disch_dt >= ce.charttime
      -- AND ce.charttime >= pt.admit_dt
...
FROM r_policytree_mimic_2023.labs;
-- 258524, 6660
