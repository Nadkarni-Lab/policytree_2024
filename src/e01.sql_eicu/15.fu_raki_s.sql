...
DROP TABLE IF EXISTS r_policytree_eicu_2023.fu_raki_s;
CREATE TABLE r_policytree_eicu_2023.fu_raki_s AS
WITH episode AS (
    SELECT pt.subject_id,
           h1.aki_stage AS h1_aki_st,
           h1.charttime AS h1_aki_dt,
           h2.aki_stage AS h2_aki_st,
           h2.charttime AS h2_aki_dt
    FROM r_policytree_eicu_2023.cohort AS pt
             INNER JOIN aims_eicu_crd_derived.kdigo_stages AS h1
                        ON pt.subject_id = h1.subject_id AND
                           pt.hadm_id = h1.hadm_id AND
                           pt.disch_dt >= h1.charttime AND
                           pt.out_dt >= h1.charttime AND
                           (pt.aki_dt + 60*48) > h1.charttime AND
             ...
             INNER JOIN aims_eicu_crd_derived.kdigo_stages AS h2
                        ON h1.subject_id = h2.subject_id AND
                           ...
                           (h1.charttime + 60*48) > h2.charttime AND
                           h2.charttime >= h1.charttime
    WHERE h1.aki_stage IS NOT NULL
      AND h2.aki_stage IS NOT NULL
), recovery0 AS (
