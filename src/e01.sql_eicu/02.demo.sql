...
-- Demographics
DROP TABLE IF EXISTS r_policytree_eicu_2023.demo;
CREATE TABLE r_policytree_eicu_2023.demo AS
SELECT pt.subject_id AS subject_id,
       p1.age        AS dm_age,
       p2.race       AS dm_race,
       p3.gender     AS dm_gender
FROM r_policytree_eicu_2023.cohort AS pt
         LEFT JOIN aims_eicu_crd_derived.age AS p1
                   ON pt.hadm_id = p1.hadm_id
         LEFT JOIN aims_eicu_crd_hosp.admissions AS p2
                   ON pt.hadm_id = p2.hadm_id
         LEFT JOIN aims_eicu_crd_hosp.patients AS p3
